...
int vertexWithMinDistance(int val)
{
    int minDist = inf;
    int index = -1;
    for(int i = 0; i < n; i++)
    {
        if(minDist > Dist[i] && Dist[i] <= val)
        {
            index = i;
            minDist = Dist[i];
        }
...
void Radix(int s)
{
    radix_heap::pair_radix_heap<int, int> Q;
    Q.push(0,s);
    //int index = 0;
    //vector<int>XD(n);
    ...
    while(!Q.empty())
    {
        //cur.x = distance, cur.y index
        //next.x next node, next.y weight
        ...
        if(Dist[cur.y] > cur.x)
        {
            ...
            {
                next =  G[cur.y][i];
                if(cur.x+next.y < Dist[next.x])Q.push(cur.x+next.y, next.x);
